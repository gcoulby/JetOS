...
#define ACTUAL_CHAR_WIDTH 8
#define ACTUAL_CHAR_HEIGHT 12
...
void renderer_draw_string(unsigned int x, unsigned int y, unsigned int scale, char *text, renderer_text_justify_t justification)
{
  ...
  if (drawing_in_progress)
  {
    ...
    case JUSTIFY_LEFT:
      ...
      {
        renderer_draw_character(x + (i * ACTUAL_CHAR_WIDTH * scale), y, scale, text[i]);
      }
      break;
    ...
    case JUSTIFY_CENTRE:
      for (int i = 0; i < length; i++)
      {
        renderer_draw_character(x - (length * scale * ACTUAL_CHAR_WIDTH / 2) + (i * ACTUAL_CHAR_WIDTH * scale), y, scale, text[i]);
      }
      break;
    ...
    case JUSTIFY_RIGHT:
      for (int i = 0; i < length; i++)
      {
        renderer_draw_character(x - ((length - i) * ACTUAL_CHAR_WIDTH * scale), y, scale, text[i]);
      }
      break;
